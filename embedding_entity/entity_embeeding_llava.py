...
def main():
    ...
    embeddings = [get_sentence_embedding(item, tokenizer, model) for item in data]
    emb_dict = {
        item['label_name']: weight_mean(embedding).tolist()
        for item, embedding in zip(data, embeddings)
    }

    with open('sentity_embedding_idx_llava.json', 'w', encoding='utf-8') as f:
        json.dump(emb_dict, f)
        print("Embeddings saved successfully.")

    emb_tensor = torch.tensor(list(emb_dict.values()), dtype=torch.float32)
    emb_tensor = emb_tensor.squeeze()
    print("emb_tensor",type(emb_tensor))
    print("emb_tensor.size()",emb_tensor.size())
    print("emb_tensor.shape",emb_tensor.shape)
    torch.save(emb_tensor, 'data/entity_embedding_idx_llava.pt')
    print("PyTorch tensor saved successfully.")
