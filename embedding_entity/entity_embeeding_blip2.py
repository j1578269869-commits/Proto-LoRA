...
def main():
    filepath = 'my_refined_updated_entities.json'
    ...
    # print(data)
    # 初始化模型和 tokenizer
    model_name = '/home/NCUT/23/jz/AlignRE_Blip/flan-t5-xxl'  # 使用正确的模型名称
    token = os.getenv("HF_TOKEN")
    # tokenizer = AutoTokenizer.from_pretrained(model_name, use_auth_use_auth_token=token)
    # model = AutoModel.from_pretrained(model_name, use_auth_use_auth_token=token,torch_dtype=torch.float16,device_map="auto")
    # model = AutoModel.from_pretrained(model_name, use_auth_use_auth_token=token,torch_dtype=torch.float16)
    tokenizer = T5Tokenizer.from_pretrained(model_name, use_auth_use_auth_token=token)
    model = T5ForConditionalGeneration.from_pretrained(model_name, use_auth_use_auth_token=token,torch_dtype=torch.float16)
    ...
    with open('embedding/extract_entity_embedding_blip.json', 'w', encoding='utf-8') as f:
    ...
    torch.save(emb_dict_list, 'embedding/extract_entity_embedding_blip.pt')
