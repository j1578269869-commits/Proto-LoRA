...
from transformers import AutoTokenizer, AutoModel, T5Tokenizer, T5ForConditionalGeneration,Qwen2VLForConditionalGeneration
import json
import torch
...
def main():
    ...
    # 构建嵌入字典
    emb_dict = {
        data[i]['relation']: weight_mean([emb])
    ...
    with open('similarity_embedding_Qwen2.json', 'w', encoding='utf-8') as f:
    ...
    torch.save(emb_dict_list, 'data/similarity_embedding_Qwen2.pt')
